...
| 年份       | 论文（简题）                                               | 任务/场景                                 | 模型与方法（关键细节）                                                                                            | 原始训练数据/数据来源                                                                      | 输入 → 输出（标签/GT）                                                                    | 结果与要点                                                                                                |
| -------- | ---------------------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
...
2023 – MiniatureVQNet: A Light-Weight DNN for Non-Intrusive Evaluation of VoIP Speech Quality

任务/场景：VoIP 语音质量的非侵入式评估，适用于嵌入式和低算力设备。

模型与方法：轻量化 DNN，包含全连接层 + 双向 GRU；设计了两种变体（Noise / Noise+Network）；采用模型量化优化。

原始训练数据：NISQA 语料库 + 噪声数据集 + 自建噪声-网络联合失真语料。

输入 → 输出：接收端语音（含环境噪声、网络损伤） → 输出 PESQ/MOS 对齐的预测分数。

结果与要点：MSE 显著低于 P.563（0.21 vs 2.19）；Noise+Network 模型泛化性更好，可稳定运行在嵌入式系统。

2024 – Evaluating RNNs for Prediction of Multi-Variate Time Series VoIP Metrics

任务/场景：LTE-A 环境下 VoIP 通话 QoS/QoE 时间序列预测。

模型与方法：VAR 预建模捕捉多变量相关性 → 输入到 RNN/LSTM/GRU 进行预测。

原始训练数据：实地采集 4 万+ RTP 包（OPUS 编码），车载 UE 驱动 70 公里路测。

输入 → 输出：六个指标（MOS、带宽、RTT、抖动、去抖缓冲、SNR） → 未来时刻对应值。

结果与要点：LSTM/GRU 优于普通 RNN，能更好捕捉时序细节；预测性能通过 MAE/RMSE/MAPE 指标量化。

2020 – An Effective ML Approach to Quality Assessment of VoIP Calls

任务/场景：对比 VQmon（E-Model 扩展）的不足，提出基于机器学习的主观 QoE 预测。

模型与方法：Ordinal Logistic Regression (OLR)，并与 DT、RF、LogReg 比较。

原始训练数据：受控实验，56 名参与者，AMR-WB 编码语料，经 NetEm 注入时延/丢包。

输入 → 输出：网络指标（抖动、丢包、乱序包等）+ 用户特征（性别、耳机类型） → 主观 MOS (ACR 1–5)。

结果与要点：OLR 准确率最高（61%），优于 RF/DT；在二分类简化任务中准确率可达 83%。

2019 – Improved VoLTE QoE Estimation using Network Metrics

任务/场景：基于 LTE 网络探针的 VoLTE QoE/MOS 估计。

模型与方法：数学模型优化 IQX 假设，引入“有效丢包率” (结合丢包、抖动、jitter buffer)。

原始训练数据：真实网络监测（PCO#1–3 接口探针），并用 POLQA 作为对照。

输入 → 输出：丢包率、抖动、jitter buffer → 优化后的 MOS（VM_MOS）。

结果与要点：VM_MOS 与 POLQA 高度接近（偏差 <0.1），优于传统 G.107 MOS。
...
MOS 并不是人工听测，而是用 E-Model (ITU G.107) 的 R-factor → MOS 映射公式计算得到
...
在 时延增加、丢包增加的实验中，RNN 的预测曲线与 PESQ 曲线几乎重合。
